...
namespace scriptium_backend_dotnet.DB
{
    ...
    public class ApplicationDBContext(DbContextOptions<ApplicationDBContext> options) : IdentityDbContext<User, Role, Guid>(options)
    {
        ...
        public async Task<List<CommentOwnerVerseDTO>> GetVerseCommentsAsync(User userRequested, long verseId, bool OrderByDecrement = false)
        {
            ...
            try
            {
                ...
                List<CommentOwnerVerseDTO> data = [];

                foreach (Comment comment in Comments)
                    data.Add(comment.ToCommentOwnerVerseDTO(userRequested));
        ...
        public async Task<List<CommentOwnerNoteDTO>> GetNoteCommentsAsync(User userRequested, long NoteId, bool OrderByDecrement = false)
        {
            SqlParameter UserIdParam = new("@UserId", userRequested.Id);
            SqlParameter NoteIdParam = new("@NoteId", NoteId);

            var Result = Comment.FromSqlRaw("SELECT * FROM dbo.GetVerseCommentHierarchy(@UserId, @NoteId)", UserIdParam, NoteIdParam).Include(c => c.User).Select(c => c.ToCommentOwnerNoteDTO(userRequested));
