...
namespace scriptium_backend_dotnet.Controllers.UserHandler
{
    ...
    public class UserController(UserManager<User> userManager, ApplicationDBContext db) : ControllerBase
    {
        ...
        public async Task<IActionResult> GetUser([FromRoute] string username)
        {
            ...
            long followedCount = await _db.Follow.CountAsync(r => r.FollowerId == UserFetched.Id && !r.Followed.IsFrozen.HasValue && r.Status == FollowStatus.Accepted);
            ...
            Follow? FollowStatusFromInspectionUserToInspectedUser = await _db.Follow.FirstOrDefaultAsync(f => f.FollowerId == UserRequested.Id && f.FollowedId == UserFetched.Id);
            Follow? FollowStatusFromInspectedUserToInspectingUser = await _db.Follow.FirstOrDefaultAsync(f => f.FollowedId == UserRequested.Id && f.FollowerId == UserFetched.Id);
        ...
        public async Task<IActionResult> GetUsersComments([FromRoute(Name = "userId")] string userFetchedUserId)
        {
            ...
            try
            {
                ...
                List<CommentBaseDTO>? data = await _db.Comment
                    ...
                    .AsSplitQuery()
                   .Select(comment => comment.ToCommentBaseDTO())
                   .ToListAsync();
        ...
        public async Task<IActionResult> GetUsersNotes([FromRoute(Name = "userId")] string userFetchedUserId)
        {
            ...
            try
            {
                ...
                List<NoteOwnDTO>? data = await _db.Note
                    ...
                    .AsSplitQuery()
                   .Select(note => note.ToNoteOwnDTO(UserRequested))
                   .ToListAsync();
